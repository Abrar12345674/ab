...
<head>
  ...
  <title>Your Cart | FOCUS HUB</title>
  <link rel="stylesheet" href="css/style.css" />

  <!-- inline cart-specific styling -->
  <style>
    .cart-item{
      background:#fff;
      border-radius:10px;
    ...
    .cart-item h3{margin:0 0 10px}
    .cart-item p{color:#555}
    .remove-btn{
    ...
    .checkout-btn{
      ...
      font-size:16px; margin-top:20px; display:block; width:100%;
    }
...
<section class="container">
  ...
  <p id="cartTotal" class="total-line"></p>

  <button id="checkoutBtn" class="checkout-btn">Proceed&nbsp;to&nbsp;Checkout</button>
</section>

<footer>
  <p>&copy; 2025 FOCUS HUB. All rights reserved.</p>
</footer>

<!-- ───────────────────────── Script A : Cart logic ───────────────────────── -->
<script>
  const API      = 'http://localhost:3030/api';
  const CART_KEY = 'focushub_cart_id';
  const CART_ID  = localStorage.getItem(CART_KEY);
  ...
  async function loadCart(){
    ...
    const items = await fetch(${API}/cart/${CART_ID}).then(r=>r.json());
    if(!items.length){
      list.innerHTML = '<p>Your cart is empty.</p>';
      total.textContent = '';
      return;
    ...
    const { total:sum } = await fetch(${API}/cart-total/${CART_ID}).then(r=>r.json());
    total.textContent = 'Total: $' + Number(sum).toFixed(2);
  }

  document.getElementById('cartItems').addEventListener('click', async e=>{
    if(!e.target.matches('.remove-btn')) return;
    await fetch(${API}/cart/item/${e.target.dataset.id}, { method:'DELETE' });
    loadCart();
  });
...
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
  <title>Checkout | FOCUS HUB</title>
  <link rel="stylesheet" href="css/style.css" />
  <style>
    .checkout-container {
      max-width: 600px;
...
<body>
  ...
  <!-- حذفنا جافاسكريبت التي تمنع الإرسال وتعرض التنبيه -->
   <script>
  const API      = 'http://localhost:3030/api';
  const CART_KEY = 'focushub_cart_id';
  const CART_ID  = localStorage.getItem(CART_KEY);
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>FOCUS HUB | Sony Cameras & Photography Tools</title>
    <style>
        body {
...
<body>
    ...
    <header>
        <h1>FOCUS HUB</h1>
        <p>Your Sony Camera & Photography Tools Destination</p>
    </header>

    <nav>
        <a href="#">Home</a>
        <a href="#sony-cameras">Sony Cameras</a>
        <a href="#photography-tools">Photography Tools</a>
        
    </nav>

    <div class="hero">
        Capture the Moment with FOCUS HUB
    </div>

    <section class="container" id="sony-cameras">
        <h2>Sony Cameras</h2>
        <div class="product-grid">
            <div class="product-card">
                <img src="camera.jpg" alt="Sony Alpha A7 III">
                <h3>Sony Alpha A7 III</h3>
                <p>$1,998.00</p>
                <button>Add to Cart</button>
            </div>
            <div class="product-card">
                <img src="camera.jpg" alt="Sony Alpha A7R IV">
                <h3>Sony Alpha A7R IV</h3>
                <p>$2,998.00</p>
                <button>Add to Cart</button>
            </div>
            <div class="product-card">
                <img src="camera.jpg" alt="Sony Alpha a6400">
    ...
    <section class="container" id="photography-tools">
        <h2>Photography Tools</h2>
        <div class="product-grid">
            <div class="product-card">
                <img src="CompactTripod.jpg" alt="Sony Tripod">
                <h3>Compact Tripod</h3>
                <p>$49.99</p>
                <button>Add to Cart</button>
            </div>
            <div class="product-card">
                <img src="ShotgunMic.jpg" alt="Sony Shotgun Microphone">
                <h3>Shotgun Mic</h3>
                <p>$129.99</p>
                <button>Add to Cart</button>
            </div>
            <div class="product-card">
                <img src="ExternalFlash.jpg" alt="Sony Flash">
...
<script>
const API      = 'http://localhost:3030/api';   // Express + SQLite back-end
const CART_KEY = 'focushub_cart_id';            // browser-side cart UUID

/* One cart ID per visitor, stored in localStorage */
function getCartId() {
  let id = localStorage.getItem(CART_KEY);
  if (!id) { id = crypto.randomUUID(); localStorage.setItem(CART_KEY, id); }
  return id;
}

/* Attach an event listener to every “Add to Cart” button */
document.querySelectorAll('.product-card button').forEach(btn => {
  btn.addEventListener('click', async () => {
    const card = btn.closest('.product-card');
    const name = card.querySelector('h3').innerText.trim();

    /* Look up product_id once, using the API list */
    const products = await fetch(${API}/products).then(r => r.json());
    const prod     = products.find(p => p.name === name);
    if (!prod) { alert('Product not found in DB'); return; }

    /* POST one unit into cart_items */
    await fetch(${API}/cart/${getCartId()}, {
      method : 'POST',
      headers: { 'Content-Type':'application/json' },
      body   : JSON.stringify({ product_id: prod.product_id, qty: 1 })
    });

    alert(${name} added to cart.);
    loadCart();                 // refresh the mini-cart list below
  });
});
</script>
...
<script>
async function loadCart() {
  const listEl  = document.getElementById('cartItems');   // you already have this <ul> in the HTML
  if (!listEl) return;                                    // safeguard in case the element is absent

  const cartId = localStorage.getItem(CART_KEY);
  if (!cartId) { listEl.innerHTML = '<p>Your cart is empty.</p>'; return; }

  const items = await fetch(${API}/cart/${cartId}).then(r => r.json());
  ...
  /* Build a simple bullet list; customize as needed */
  listEl.innerHTML = items.map(i =>
    <li>${i.name} – $${i.price.toFixed(2)} × ${i.qty}</li>
  ).join('');
}
...
document.querySelectorAll('button').forEach(btn => {
  btn.addEventListener('click', async function () {
    ...
    /* lookup product_id once per click */
    const products = await fetch(${API}/products).then(r => r.json());
    const prod     = products.find(p => p.name === name);
    if (!prod) { alert('Product not found in DB'); return; }

    /* insert/append to cart_items */
    await fetch(${API}/cart/${getCartId()}, {
      method : 'POST',
      headers: { 'Content-Type':'application/json' },
      body   : JSON.stringify({ product_id: prod.product_id, qty: 1 })
...
INSERT INTO products (name, price) VALUES
 ('Sony Alpha A7 III', 1998.00),
 ('Sony Alpha A7R IV', 2998.00),
 ('Sony Alpha a6400',   898.00),
 ('Compact Tripod',      49.99),
 ('Shotgun Mic',        129.99),
 ('External Flash',     249.99);
{
  "name": "focus_hub_api",
  "version": "1.0.0",
  "type": "module",               
